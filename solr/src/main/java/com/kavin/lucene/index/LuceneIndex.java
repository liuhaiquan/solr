package com.kavin.lucene.index;
...
public class LuceneIndex {
    ...
    public static void indexCreate(File targetFileDir, File indexSaveDir) throws IOException {
         ...
         * Lucene 5.5.5 版本 open 方法传入的 Path 对象
         * Lucene 4.10.3 版本 open 方法传入的是 File 对象
         */
        Directory directory = FSDirectory.open(indexSaveDir.toPath());

        /** 创建 索引写配置对象，传入分词器
         * Lucene 5.5.5 版本 IndexWriterConfig 构造器不需要指定 Version.LUCENE_47
         * Lucene 4.10.3 版本 IndexWriterConfig 构造器需要指定 Version.LUCENE_47
         * */
         ...
         * Lucene 5.5.5 可以自己手动调用 flush 方法*/
        indexWriter.commit();
        indexWriter.close();
